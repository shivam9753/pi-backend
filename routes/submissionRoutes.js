...
router.post('/:id/publish-with-seo', authenticateUser, requireReviewer, validateObjectId('id'), async (req, res) => {
  try {
    ...
    // Ensure we have a submissionMeta object (accept nested structure)
    const submissionMeta = incoming.submissionMeta && typeof incoming.submissionMeta === 'object' ? { ...incoming.submissionMeta } : {};

    // Ignore any legacy featuredPost flag — we do not persist it
    if ('featuredPost' in submissionMeta) delete submissionMeta.featuredPost;

    // If a primaryKeyword provided at top-level (legacy), prefer that
    if (!submissionMeta.primaryKeyword && incoming.primaryKeyword) {
      submissionMeta.primaryKeyword = String(incoming.primaryKeyword).trim();
    }

    // Normalize per-content meta
    const perContentMeta = incoming.perContentMeta && typeof incoming.perContentMeta === 'object' ? { ...incoming.perContentMeta } : {};

    // Build normalized payload for service — include submission-level title/description/excerpt if provided
    const normalized = {
      ...
      submissionMeta,
      perContentMeta,
      perContentTags: incoming.perContentTags || undefined
    };

    // Call the service with normalized payload
    const submission = await SubmissionService.publishWithSEO(req.params.id, normalized, req.user._id);
...
router.get('/by-slug/:slug', async (req, res) => {
  try {
    ...
    // Build a lightweight public payload for rendering the full post
    ...
    const contents = Array.isArray(submission.contents) ? submission.contents.map(c => ({
      _id: c._id,
      title: c.title,
      body: c.body,
      footnotes: c.footnotes || '',
      // Normalize tags to {_id, name, slug}
      tags: Array.isArray(c.tags) ? c.tags.map(t => ({ _id: t && t._id ? String(t._id) : null, name: t && t.name ? t.name : '', slug: t && t.slug ? t.slug : '' })) : []
    })) : [];
    ...
    return res.json({ success: true, submission: light });
  } catch (error) {
    if (error.message === 'Published submission not found') {
      return res.status(404).json({ message: error.message });
    }
